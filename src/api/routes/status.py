"""Workflow status route handler."""

from __future__ import annotations

from ..concurrency import _run_sync
from ..constants import ROUTE_STATUS, SPAN_STATUS
from ..errors import ApiError
from ..mappers import map_workflow_status
from ..protocols import ApiDependencies
from ..telemetry import get_active_telemetry
from ..types import WorkflowStatusResponse
...
async def handle_get_workflow_status(
    *,
    ...
    """GET /workflows/{workflow_id} — query current workflow state (ACD-API-002)."""
    telemetry = get_active_telemetry()
    validator = RequestValidator()
    wf_id = validator.validate_workflow_id(workflow_id)

    span = telemetry.start_route_span(SPAN_STATUS, attributes={"workflow_id": wf_id})
    with span:
        try:
            status = await _run_sync(
                lambda: deps.workflow_engine.get_workflow_status(wf_id),
            )
        ...
        return map_workflow_status(status)
