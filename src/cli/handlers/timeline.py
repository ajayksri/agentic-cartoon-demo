...
class TimelineHandler(BaseSubcommandHandler):
    """Handles workflow timeline queries."""
    ...
    def _execute(self, ctx: CliCommandContext) -> None:
        ...
        response = self._async_runner.run(
            ctx.api_client.get_workflow_timeline(workflow_id)
        )
        self._renderer.render_timeline(response)
